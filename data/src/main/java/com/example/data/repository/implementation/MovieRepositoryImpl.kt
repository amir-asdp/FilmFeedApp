...
class MovieRepositoryImpl(
    ...
    override fun getMoviesOrderByRate(): Flow<PagingData<MovieBrief>> {
        return Pager(
            ...
            pagingSourceFactory = { MovieOrderByRatePagingSource(movieRemoteDataSource, coroutineDispatcher) }
        ).flow
    }
    ...
    override fun searchMovies(searchQuery: String): Flow<PagingData<MovieBrief>> {
        return Pager(
            config = PagingConfig(20, 2, true),
            pagingSourceFactory = { MovieSearchPagingSource(movieRemoteDataSource, coroutineDispatcher, searchQuery) }
        ).flow
    }
